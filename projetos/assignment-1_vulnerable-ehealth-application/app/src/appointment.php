<?php
	...
	if($userLogged && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['searchAppointment'])){
		...
		if(!empty($_POST['appointment_date']) && validateInputs($doctorId, "int"))
			$bookedDate = $_POST['appointment_date'];
        else
            header("Location: ./appointment.php?spec=".$specialtyId."&msg=3");
                ...
                ?>
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        <h4>
                            ...
                            BOOK APPOINTMENT <?php echo "(".$specialtyName.")"; ?>
                        </h4>
                        <div class="form-row ">
									...
									<?php
										...
										if($doctors->num_rows > 0){
											while($row = $doctors->fetch_assoc()){
												...
												echo "<option value='".$doctor_info['id']."'>".$doctor_info['first_name']." ".$doctor_info['last_name']."</option>";
											}
										}
									?>
                                </select>
						...
						for($i = 9; $i < 18; $i++){
							if(!isset($dateArray[$arrIndex]) || !(substr($dateArray[$arrIndex], 11, 2) == $i)){
								...
								echo "<tr>
										...
										<td>
											<a class='btn btn-outline-dark' role='button' href='?spec=".$specialtyId."&doctor=".$doctorId."&date=".$newDate."' >BOOK</a>
										</td>
									</tr>";
							}else
								$arrIndex++;
